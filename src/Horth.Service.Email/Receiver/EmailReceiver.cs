...
namespace Horth.Service.Email.Service
{
    ...
    public class EmailReceiver : MessageNotificationSubscriber<IrcMessageQueueMessage>, IHostedService
    {
        ...
        private readonly IrcMessageQueueReceiver _messageQueueReceiver;
        private readonly IServiceProvider _serviceProvider;

        public EmailReceiver(IrcMessageQueueReceiver messageQueueReceiver, ISendMessageService sender, IServiceProvider serviceProvider) 
            : base(IrcMessageQueueMessage.MsgService.Email.ToString(), true)
        {
            _sender = sender;
        ...
        public override bool HandleMessage(IrcMessageQueueMessage msg)
        {
            ...
            {
                using (var db = scope.ServiceProvider.GetService<IEmailUnitOfWork>())
                {
                    db.Email.Log(emailMsg, (int)ret.Outcome);
                }
            }
        ...
        {
            _messageQueueReceiver.Initialize(IrcMessageQueueMessage.MsgService.Email);
